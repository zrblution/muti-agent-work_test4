...
After this read-only report, the main implementation added the structured `run-landmark` validation gate and then added `experiments/landmark_baselines/run_landmark.py` as the whitelisted worker entry point. The worker entry point is non-recursive and now includes the reviewed execution loop. It first records validation-gate failures when paths, inventory, or Qwen runtime dependencies are missing; with monkeypatched runtime adapters it writes the complete success artifact set. The implementation also added `phase5-discover-model-candidates`, a bounded read-only model path discovery command for auditing candidate Qwen roots before `phase5-probe-paths`. The real Qwen3-VL + POPE smoke is still not accepted until approved server paths, dependencies, and execution gates produce a validated real bundle.
