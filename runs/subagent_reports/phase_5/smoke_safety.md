...
After this read-only report, the main implementation added the structured `run-landmark` validation gate and then added `experiments/landmark_baselines/run_landmark.py` as the whitelisted worker entry point. The worker entry point is non-recursive and now includes the reviewed execution loop. It first records validation-gate failures when paths or inventory are missing; with placeholder inventory it records `failure_type: landmark_worker_execution_failed`; with monkeypatched runtime adapters it writes the complete success artifact set. The real Qwen3-VL + POPE smoke is still not accepted until approved server paths, dependencies, and execution gates produce a validated real bundle.
