...
The command now supports `--output-dir`, writing `phase5_gate_audit.json` plus `phase5_gate_audit.md` for human review. The package includes a structured `next_action_packet` for the next missing gate, with required inputs, safe command templates, expected artifacts, and forbidden actions. The packet now covers the full non-executing chain: initial model-path decision-request creation, model-path decision validation after a pending request exists, approved-decision readiness after a validated approval exists, config-representation proposal after approved-decision readiness exists, config-representation decision validation after the proposal exists, Phase 5 readiness after config decision validation exists, and final real-smoke evidence handoff after readiness passes. The later packets keep approved paths out of config/env mutation until a validated representation decision, keep readiness in read-only plan mode, and require recorded worker evidence validation before any final Phase 5 outcome is accepted.

The current committed handoff package is stored under `runs/needs_attention/phase_5_gate_audit_current/`. It was generated from the committed pending model-path decision request and points the reviewer to `model_path_decision_validation`, the filled human decision-record input, and the non-executing validation command while preserving all execution safety flags as false. The package now records supplied source artifacts with sha256 hashes so the reviewer can detect stale handoffs before validating a human decision record.

`phase5-verify-gate-audit` now verifies a recorded gate-audit package before it is used for human action. It checks package identity, non-executing safety fields, the structured `next_action_packet`, each recorded source artifact's current existence and sha256, and the sibling Markdown sidecar's critical handoff fields plus packet lists. A stale JSON/source-artifact or JSON/Markdown mismatch fails with explicit drift evidence instead of relying on manual comparison.

`phase5-decision-record-status` now inspects model-path decision-record candidates before the non-executing validator is run. With `--audit`, it also verifies the current gate-audit handoff and blocks readiness when source-artifact or Markdown sidecar evidence is stale. With `--output-dir`, it writes a reviewable JSON plus Markdown package. It keeps committed unfilled templates and zero-candidate directories in `needs_attention`, reports invalid or multiple filled candidates as failed handoffs with decision-type counts, and treats exactly one filled candidate plus a current audit as ready only for `phase5-validate-model-path-decision`, not as approval or run authorization.

The current committed scanner output is stored under `runs/needs_attention/phase_5_decision_record_status_current/`. It records the pending decision request, the unfilled template directory, and the current gate audit in one read-only JSON plus Markdown handoff; the status remains `needs_attention` because there are zero filled candidates even though the gate-audit verification passes.

`phase5-verify-decision-record-status` now verifies the recorded decision-record status package before human action. It checks package identity, non-executing safety flags, current request/records/audit paths, current gate-audit verification, and Markdown sidecar consistency. This lets the reviewer reject stale status packages before relying on the handoff.
...
`phase5-prepare-decision-workspace` now prepares fillable copies of the three decision templates after verifying the current handoff. The committed workspace under `runs/needs_attention/phase_5_human_decision_workspace_current/` remains unfilled and non-executing, with `expected_fill_count: 1` and all execution safety flags false. `phase5-decision-record-status` also treats unselected unfilled workspace copies as unfilled handoff records, so one filled prepared record can proceed to non-executing validation without requiring the reviewer to delete the other two prepared options first.
