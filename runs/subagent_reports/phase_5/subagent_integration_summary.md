...
`phase5-verify-gate-audit` now verifies a recorded gate-audit package before it is used for human action. It checks package identity, non-executing safety fields, the structured `next_action_packet`, each recorded source artifact's current existence and sha256, and the sibling Markdown sidecar's critical handoff fields. A stale JSON/source-artifact or JSON/Markdown mismatch fails with explicit drift evidence instead of relying on manual comparison.
