...
`phase5-verify-gate-audit` now verifies a recorded gate-audit package before it is used for human action. It checks package identity, non-executing safety fields, the structured `next_action_packet`, and each recorded source artifact's current existence and sha256. A stale handoff fails with expected and actual sha256 values instead of relying on manual comparison.
