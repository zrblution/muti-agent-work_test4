...
- `phase5-decision-record-status` can now inspect a model-path decision records directory before validation. It distinguishes unfilled templates, filled candidates, and invalid candidates without approving a path, mutating config, exporting env vars, opening gates, loading a model, running a benchmark, submitting a job, or writing raw outputs.
...
- After filling exactly one copied template, run `phase5-decision-record-status --request runs/needs_attention/phase_5_model_path_decision_request/phase5_model_path_decision_request.json --records-dir <records_dir> --output <status_report.json>` and require exactly one filled candidate before running `phase5-validate-model-path-decision`.
