...
- If the committed variant packet is being considered, review `runs/needs_attention/phase_5_model_path_decision_request/phase5_model_path_decision_request.json` and fill one matching `decision_record_templates` entry for `approve_variant_path`, `reject_variant_path`, or `provide_base_model_root`.
