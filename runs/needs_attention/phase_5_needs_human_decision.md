...
- `phase5-decision-record-status` can now inspect a model-path decision records directory before validation. With `--audit`, it also verifies the current gate-audit handoff still points to `model_path_decision_validation`, and with `--output-dir` it writes a reviewable JSON plus Markdown package. It distinguishes unfilled templates, exactly one filled candidate, multiple filled candidates, invalid candidates, and stale audit/source evidence without approving a path, mutating config, exporting env vars, opening gates, loading a model, running a benchmark, submitting a job, or writing raw outputs. When multiple filled candidates exist, it reports decision-type counts and `ambiguous_decisions` so the human can remove extras deterministically.
- `runs/needs_attention/phase_5_decision_record_status_current/` now stores the committed current status package for the unfilled decision-record templates plus the current gate-audit package. The JSON and Markdown sidecar report `needs_attention`, zero filled candidates, three unfilled templates, passed gate-audit verification, `ready_for_decision_validation: false`, `ready_for_real_smoke: false`, and all execution safety flags false.
- `phase5-verify-decision-record-status` can now verify a recorded decision-record status package before human action. It checks package identity, non-executing safety flags, current request/records/audit paths, current gate-audit verification, and the Markdown sidecar without approving a path, mutating config, exporting env vars, opening gates, loading a model, running a benchmark, submitting a job, or writing raw outputs.
- `runs/needs_attention/phase_5_decision_record_status_verification_current/phase5_decision_record_status_verification.json` now stores the committed current verification result for that status package. It reports `passed` while preserving `ready_for_real_smoke: false` and all execution safety flags false.
- `phase5-current-handoff` can now verify the committed gate-audit package and decision-record status package together, then write one JSON plus Markdown handoff. `runs/needs_attention/phase_5_current_handoff/` stores the current package with `status: needs_attention`, `verification_status: passed`, next missing gate `model_path_decision_validation`, `ready_for_decision_validation: false`, `ready_for_real_smoke: false`, and all execution safety flags false.
- `phase5-prepare-decision-workspace` can now prepare fillable copies of the three decision templates under a separate `decision_records/` directory after verifying the current handoff. `runs/needs_attention/phase_5_human_decision_workspace_current/` stores the current unfilled workspace with `expected_fill_count: 1`, zero filled candidates, `ready_for_decision_validation: false`, `ready_for_real_smoke: false`, and all execution safety flags false.
...
- `runs/needs_attention/phase_5_gate_audit_current/` now stores the current committed gate-audit package for the pending model-path decision request. It reports `next_missing_gate: model_path_decision_validation`, lists the filled human decision-record input, records the decision-request source path and sha256, and points to the non-executing `phase5-validate-model-path-decision` handoff.
- `phase5-verify-gate-audit` can now verify a recorded gate-audit package before human action. It checks package identity, non-executing safety flags, `next_action_packet`, current source-artifact path/hash provenance, and the sibling Markdown sidecar's critical handoff fields plus packet lists without editing config, exporting env vars, opening gates, loading a model, running a benchmark, submitting a job, or writing raw outputs.
...
- Review `runs/needs_attention/phase_5_gate_audit_current/phase5_gate_audit.md` as the current committed gate handoff before filling or validating any human decision record; first run `phase5-verify-gate-audit --audit runs/needs_attention/phase_5_gate_audit_current/phase5_gate_audit.json --output <verification_report.json>` and require `status: passed`, including the Markdown sidecar check.
- Review `runs/needs_attention/phase_5_decision_record_status_current/phase5_decision_record_status.md` as the committed current decision-record status before filling a copied template. Also inspect `runs/needs_attention/phase_5_decision_record_status_verification_current/phase5_decision_record_status_verification.json`, then rerun `phase5-verify-decision-record-status --status runs/needs_attention/phase_5_decision_record_status_current/phase5_decision_record_status.json --output <status_verification_report.json>` and require `status: passed`. The status package itself is expected to remain `needs_attention` until exactly one human-filled candidate exists.
- Review `runs/needs_attention/phase_5_current_handoff/phase5_current_handoff.md` as the consolidated current handoff, then rerun `phase5-current-handoff --gate-audit runs/needs_attention/phase_5_gate_audit_current/phase5_gate_audit.json --decision-status runs/needs_attention/phase_5_decision_record_status_current/phase5_decision_record_status.json --output-dir <current_handoff_report_dir>` and require `verification_status: passed` while expecting top-level `status: needs_attention`.
- Review `runs/needs_attention/phase_5_human_decision_workspace_current/phase5_human_decision_workspace.md`, then fill exactly one JSON file under `runs/needs_attention/phase_5_human_decision_workspace_current/decision_records/`. Leave the other two prepared records unfilled, or remove them, but do not fill more than one.
- After filling exactly one prepared decision record, run `phase5-decision-record-status --request runs/needs_attention/phase_5_model_path_decision_request/phase5_model_path_decision_request.json --records-dir runs/needs_attention/phase_5_human_decision_workspace_current/decision_records --audit runs/needs_attention/phase_5_gate_audit_current/phase5_gate_audit.json --output-dir <status_report_dir>` and require exactly one filled candidate, no invalid candidates, and a passed gate-audit verification before running `phase5-validate-model-path-decision`; multiple filled candidates must be reduced to one before validation.
