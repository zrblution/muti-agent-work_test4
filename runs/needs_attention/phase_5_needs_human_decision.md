...
- Review `runs/needs_attention/phase_5_decision_record_status_current/phase5_decision_record_status.json` as the committed current decision-record status before filling a copied template. It is expected to remain `needs_attention` until exactly one human-filled candidate exists.
- After filling exactly one copied template, run `phase5-decision-record-status --request runs/needs_attention/phase_5_model_path_decision_request/phase5_model_path_decision_request.json --records-dir <records_dir> --audit runs/needs_attention/phase_5_gate_audit_current/phase5_gate_audit.json --output <status_report.json>` and require both exactly one filled candidate and a passed gate-audit verification before running `phase5-validate-model-path-decision`; multiple filled candidates must be reduced to one before validation.
