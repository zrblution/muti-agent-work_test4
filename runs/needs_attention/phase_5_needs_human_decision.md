...
- The whitelisted worker entry point exists, is non-recursive, and now records `landmark_worker_not_implemented` after validation passes because Qwen3-VL and POPE runtime methods are available but not yet connected into a real execution loop. The worker still does not load models or run benchmarks.
...
- Implement the reviewed worker execution loop that calls Qwen3-VL and POPE runtime methods before enabling process submission for a real smoke.
