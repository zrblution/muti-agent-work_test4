...
- `phase5-decision-record-status` with two temporary filled approval records: `failed`, `filled_candidate_count: 2`, no selected decision-record path, `ready_for_decision_validation: false`, `ready_for_real_smoke: false`, no config write, no env export, and all safety flags false
