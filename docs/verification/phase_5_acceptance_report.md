...
Qwen3-VL runtime update: `Qwen3VLAdapter` now implements delayed-import `load`, `generate`, and `unload` methods for approved local model paths. Loading forces `local_files_only: true`, uses configured precision/device settings, and does not run during validation or readiness checks. Generation constructs a local-image multimodal chat request, decodes only newly generated tokens, and preserves raw text in `GenerationOutput`.
...
- `RemoteRunner.submit(...)` with remote mode, GPU budget, process submission, temporary placeholder inventory, and a temporary run root: submits the whitelisted worker subprocess, exits code `1`, returns JSON status `needs_attention`, records `failure_type: landmark_worker_execution_failed`, preserves diagnostics, and writes no raw outputs
...
- direct `experiments/landmark_baselines/run_landmark.py` worker invocation with temporary placeholder model and benchmark inventory: exit code `1`, JSON status `needs_attention`, failure type `landmark_worker_execution_failed`, gate failure `worker-execution`, no raw outputs
...
- Configure approved local model and POPE paths with runtime dependencies, then run the non-recursive worker through the reviewed RemoteRunner gate to produce either a validated success bundle or a reviewed real-execution failure bundle.
