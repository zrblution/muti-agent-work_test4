...
Decision-record status update: `phase5-decision-record-status --request <decision_request.json> --records-dir <records_dir> [--audit <phase5_gate_audit.json>] --output <report.json>` now scans a directory of candidate model-path decision records before validation. It classifies unfilled templates, filled candidates, and invalid candidates, requiring exactly one filled candidate before the reviewer proceeds to `phase5-validate-model-path-decision`. With `--audit`, it also verifies the current gate-audit handoff and requires that audit to point to `model_path_decision_validation`; stale audit/source evidence blocks readiness even when a candidate record is filled. This is read-only and does not approve a path, mutate config, export env vars, open gates, read `.env`, load weights, run generation, submit jobs, run benchmarks, or write raw outputs.
