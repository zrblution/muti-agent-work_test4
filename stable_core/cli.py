...
def build_parser() -> argparse.ArgumentParser:
    ...
    phase5_model_path_decision_parser = subparsers.add_parser(
        "phase5-model-path-decision-request",
        help="Write a pending human decision packet for a Phase 5 explicit model path without approving or executing it.",
    )
    phase5_model_path_decision_parser.add_argument("--model", required=True)
    phase5_model_path_decision_parser.add_argument("--benchmark", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "phase5-probe-explicit-model-path":
        ...
        return _exit_code(str(report["status"]))
    if args.command == "phase5-model-path-decision-request":
        try:
            report = build_phase5_model_path_decision_request(
                model_id=args.model,
                benchmark_id=args.benchmark,
                model_path=args.model_path,
                benchmark_root=args.benchmark_root,
            ...
            print(json.dumps({"command": "phase5-model-path-decision-request", "status": "failed", "error": str(exc)}, ensure_ascii=False))
            return 1
        safety_flags = report["safety_flags"]
        print(
            json.dumps(
                {
                    "command": "phase5-model-path-decision-request",
                    "status": report["status"],
